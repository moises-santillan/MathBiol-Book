...
This tells us how far $x(t)$ is from the steady state $x^*$ at any time $t$. For our analysis to work, $\delta x(t)$ needs to converge to zero as time passes. Hence, we can write the stability condition in terms of $\delta x(t)$ as:
```{math}
:label: eq:06.05
...
## Discussion
In this chapter, we introduced graphical methods and local stability analysis to determine the stability of steady states in univariate nonlinear systems. By linearizing near these states, we confirmed the stability criterion established graphically: a steady state is stable if and only if the derivative of the governing function is negative at that point. While graphical analysis is intuitive, linearization offers a systematic framework that generalizes to higher-dimensional systems where visual representation is impossible. This analytical approach allows us to quantify long-term behavior near equilibria for models of any complexity, providing a foundation for the multi-species and ecosystem models discussed in subsequent chapters.
...
1. Find of the fixed points of the following differential equations and classify their stability.
    ...
    8. $\dot{x} = e^{-x} \sin(x)$.
    9. $\dot{x} = 1 - 2 \cos(x)$.
    10. $\dot{x} = (x-1)(2-x)(x-3)$.
    11. $\dot{x} = 9 - x^2$.
    12. $\dot{x} = x \ln\left(\frac{x+1}{2}\right)$.
...
1. For every one of the following assertions, find an equation of the form $\dot{x} = f(x)$ (with $f(x)$  a continuous function) that satisfies it. If no example exists, explain why not.
    ...
    2. Every integer number is a fixed point, and there are no others.
    3. There are precisely 3 fixed points, and all of them are stable.
    4. There are precisely 3 fixed points, one is stable and the other two are unstable.
    5. There are no fixed points.
