...
## Spring-mass system
We begin our examination of the damped harmonic oscillator by considering the classical spring-mass system. As shown in Fig. :numref:`fig:18.01`, this consists of a point mass $m$ attached to an ideal linear spring with stiffness constant $K$. The spring exerts a restoring force $F_s$ proportional to the displacement $x$ of the mass from its equilibrium position, according to Hooke's law: $F_s = -Kx$.
...
## Pendulum
We next consider how the damped harmonic oscillator model applies to small oscillations of a physical pendulum. As illustrated in Fig. :numref:`fig:18.02`, this consists of a point mass $m$ hanging from a rigid rod of length $L$, which is fixed at one end to a pivot.
...
## RLC Circuit
We now show that oscillatory behavior in an electrical RLC circuit is also described by the damped harmonic oscillator model. Consider the series circuit in Fig. :numref:`fig:18.03` consisting of a resistor R, inductor L, and capacitor C.
