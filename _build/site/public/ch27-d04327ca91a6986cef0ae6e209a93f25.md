...
Graphical analysis (Fig. :numref:`fig:27.01`) reveals that for large $K$, this equation has a single solution $p=0$ (no expression). As $K$ decreases, two additional steady states emerge via a saddle-node bifurcation: an intermediate state and a high expression state near $p=1$.
...
1. Consider the following toy model of a gene regulatory network subject to positive feedback regulation:
$$
