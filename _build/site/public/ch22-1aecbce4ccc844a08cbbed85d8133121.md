# 22. Quasisteady State Approximation
...
In this section, we examine the solution to the generalized response equation {eq}`eq:22.01` for some important cases of the input $\bar{x}(t)$. This provides insight into how the system responds under different input conditions.

### Constant Input
When the input is held at a constant value, the integral term in the solution {eq}`eq:22.02` evaluates to simply $\bar{x}[\exp(\gamma t) - 1]$. Substituting this into the solution yields:
$$
...
First, we analyze the reduced 1D quasi-steady state approximation given by Eq. {eq}`eq:22.05`. At steady state, the time derivative must equal zero, yielding the steady state condition $x^* = f(g(x^*))$.

To assess the stability of the system, we can perform a linear stability analysis. The Jacobian and its single eigenvalue are simply the partial derivative of the right-hand side of Eq. {eq}`eq:22.05` with respect to $x$, evaluated at the equilibrium point. That is, 
```{math}
:label: eq:22.06
...
Using the large timescale separation assumption of $\gamma \gg 1$, we obtain the following simplified expressions for the eigenvalues—see Eq. {eq}`eq:22.06`:
$$
