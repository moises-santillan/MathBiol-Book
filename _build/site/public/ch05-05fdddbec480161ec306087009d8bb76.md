...
Based on the findings presented in the previous section of Chapter :ref:`ch:03`, and assuming that the probability density $\rho(\tau, t) = \nu(\tau, t) / N(t)$ remains stationary, we can integrate Eq. :eq:`eq:05.01` to obtain the following expression:
```{equation} eq:05.02
...
Now, let's explore how long it would take for the culture's mass to reach the magnitude of planet Earth ($M_E \approx 6 \times 10^{24}$ kg). Given that an individual bacterium weighs approximately $10^{-15}$ kg and an \textit{E. coli} culture can double its mass every 30 minutes, we can solve for $t$ in Eq. :eq:`eq:05.05`:
$$
...
will exhibit geometric behavior ($y_{i+1} = 2 y_i$). The remarkable growth potential of geometric sequences has captivated humanity for centuries, giving rise to legends like the one illustrated in Chapter :ref:`ch:02`.
