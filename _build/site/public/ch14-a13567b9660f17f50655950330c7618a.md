...
Rather than computing the eigenvalues directly, which could yield cumbersome expressions, we analyze the Jacobian trace ($\tau$) and determinant ($\Delta$) as explained in Chapter~:ref:`ch:11` and summarized in Fig.~ref`fig:11.01`:
$$
\tau = -R_0,
$$
