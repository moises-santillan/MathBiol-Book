...
We have shown that the disease-free equilibrium (DFE) is stable when $R_0\eta<1$, resulting in disease extinction, but unstable for $R_0\eta>1$, allowing potential outbreaks. To further characterize outbreak dynamics, we analyze the behavior near the DFE. Considering the Jacobian in Equation ({eq}`eq:16.07`), linearization of Equations ({eq}`eq:16.04`)-({eq}`eq:16.06`) yields:
$$
