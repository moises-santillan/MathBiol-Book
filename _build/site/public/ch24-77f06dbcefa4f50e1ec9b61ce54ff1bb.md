...
For the reversible binding reaction in Eq. {ref}`eq:24.01`, application of the law of mass action gives:
$$
...
```{math}
:label: eq:24.03
K_D = \frac{k_{\text{off}}}{k_{\text{on}}} = \frac{[\text{L}][\text{R}]}{[\text{R}_L]}
```

Parameter $K_D$ is known as the equilibrium or dissociation constant, and characterizes the equilibrium of the reversible binding reaction.

## Modeling Ligand Binding to a Receptor with Single Binding Site
We consider the case where the receptor $\text{R}$ has a single binding site for ligand $\text{L}$. The reversible binding reaction is given by Eq. {ref}`eq:24.01`.
...
where $K_A \leq 1$ is the cooperativity factor and $K_D$ is the affinity of first site.
...
This equation resembles the Michaelis-Menten equation but has an initial zero slope and a sigmoidal shape. The steepness at $[\text{L}] = K_H$, where $\sigma = 1/2$, depends on $n$. Large $n$ values give nearly switch-like sigmoidal behavior---see the bottom panel of Fig. {ref}`fig:24.01`.
