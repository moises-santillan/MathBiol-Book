...
$$
K_D = n \frac{[\text{L}]\overline{[\text{R}]}}{\overline{[\text{R}_L]}} = \frac{n-1}{2} \frac{[\text{L}]\overline{[\text{R}_L]}}{\overline{[\text{R}_{2L}]}} = \cdots = \frac{1}{n}\frac{[\text{L}]\overline{[\text{R}_{(n-1)L}]}}{\overline{[\text{R}_{nL}]}}.
$$

This represents a system of $n$ equations with $n+1$ unknowns. When completed with the conservation equation,
$$
\overline{[\text{R}]} + \overline{[\text{R}_L]} + \cdots + \overline{[\text{R}_{nL}]} = [\text{R}_T],
$$

it can be solved to obtain steady state concentrations of all receptor states:
$$
\overline{[\text{R}_{mL}]} = [\text{R}_T] \frac{m!}{(n-m)! n!}\frac{([\text{L}]/K_D)^m}{(1 + \text{[\text{L}]}/K_D)^n} \quad \forall m=1 \text{ to } n.
$$
