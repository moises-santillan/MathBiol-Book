...
The above analysis provides insight into why microscopic droplets present in clouds do not readily fall under the force of gravity. Eq. {eq}`eq:02.0` demonstrates that the relaxation time $\tau$ is directly proportional to the square of the droplet radius. Given typical cloud droplet sizes on the order of 10 $\mu$m, this results in relaxation times that are exceedingly short; on the order of milliseconds based on standard fluid properties.
