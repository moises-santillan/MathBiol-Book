...
The total population size is non-dimensionalized to $1$ by defining normalized variables $s=S/N$, $i=I/N$, $r=R/N$—with $N=S+I+R$  the total population size—and a rescaled time $t'=\gamma t$. In terms of these, the equations become:
``` {math}
:label: eq:16.04
\begin{align}
\frac{ds}{dt'}  &= -R_0 s i, \\
...
In a perfectly "naive" population—where every individual is susceptible ($\eta = 1$)—the condition for an outbreak simplifies to the foundational threshold: $R_0 > 1$. 

However, we can solve for the critical susceptible threshold, $s^*_c$, required to prevent an invasion in a partially immune population:
$$s^*_c = \eta < \frac{1}{R_0}.$$

This inequality defines the mathematical basis for herd immunity. By recalling that the immune fraction is $r = 1 - \eta$, we derive the minimum proportion of the population that must be immune to ensure any introduced infection withers away:
$$r^*_c > 1 - \frac{1}{R_0}.$$

When this threshold is surpassed, the population has achieved a state where the "chain of transmission" is broken. Even though susceptible individuals remain, the density of hosts is too low to sustain the growth of the pathogen, causing the infection rate to decline naturally.
...
By substituting the initial state $s(0) = \eta$, we find the initial effective reproduction number $R_{e, 0} = R_0 \eta$. As established in the stability analysis, the population is protected from an epidemic if $R_{e, 0} < 1$, which defines the critical threshold for the susceptible fraction: $\eta < 1/R_0$.

From Eq. {eq}`eq:16.04`, it is evident that the infectious fraction $i(t)$ grows as long as $R_e(t) > 1$ and begins to decline the moment $R_e(t) < 1$. During a typical outbreak, $s(t)$—and consequently $R_e(t)$—declines monotonically as individuals contract the disease and move into the recovered class. 

The moment $R_e(t) = 1$—the herd immunity threshold—is the peak of the epidemic curve; however, it is a common misconception that the outbreak ends at this point. Because a significant portion of the population remains infectious ($i(t) > 0$) when the threshold is crossed, the "momentum" of the disease carries the outbreak forward. This leads to epidemic overshoot, where the final number of susceptible individuals is depleted far below the herd immunity level before the infection finally dies out.
