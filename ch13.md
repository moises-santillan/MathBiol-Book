...
3. For the following 2-dimensional dynamical system, plot the bifurcation diagrams showing the equilibrium states ($x^*$ and $y^*$) as a functions of the parameter $r$. Use distinct line styles to differentiate between stable and unstable fixed points:
    * $\dot{x} = x(1-x-y/2)$, 
      $\dot{y} = y(1-y-rx)$.
