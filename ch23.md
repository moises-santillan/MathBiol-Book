...
The steady state condition for the reduced dynamical system {eq}`eq:23.03` is $dE/dt=0$, which transforms into
$$
...
Substituting the expression for $k_e$ in {eq}`eq:23.04` yields:
$$
